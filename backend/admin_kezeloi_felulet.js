...
router.get("/allproblems", authenticateToken, async (req, res) => {
  ...
  try {
        const [rows] = await db.query(`
            SELECT p.problem_id, p.helyszin, p.idopont, p.leiras, p.kep_url, p.assigned_to,  CONCAT(a.vezeteknev, ' ', a.keresztnev) AS assigned_name, p.status, p.ugyfelszolg_megjegy, CONCAT(u.vezeteknev, ' ', u.keresztnev) AS user
            FROM problems p
